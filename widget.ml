...
let make_controller (v: 'a) : 'a value_controller =
  let list = ref [] in
  let value = ref v in
  { add_change_listener = (fun f -> list := !list @ [f]);
    get_value = (fun () -> !value);
    change_value = (fun x -> value := x; 
                    List.iter ((fun f -> f x)) !list)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  ...
  let w = {
    repaint = (fun (g: Gctx.gctx) ->
               Gctx.draw_rect g (0,0) (19,19);
              (if vc.get_value () then (Gctx.draw_line g (0,0) (19,19); 
                                        Gctx.draw_line g (0, 19) (19, 0))
              else (Gctx.draw_line g (0,0) (0,0); 
                    Gctx.draw_line g (0, 0) (0, 0))));
    handle = (mouseclick_listener (fun () -> 
              vc.change_value (not (vc.get_value ()))));
    size = fun () -> (20, 20)
  } in
  let w2 = hpair w l in
  (w2, vc)
...
(* Creates a slider widget that stores a given integer value *)

let slider (r: int) : widget * int value_controller =
    if r >= 1 then 
    (
    let vc = make_controller r in
    let w = {
      repaint = (fun (g: Gctx.gctx) ->
                 Gctx.draw_rect g (0, 0) (99, 19);
                 let c = Gctx.with_color g Gctx.black in
                 Gctx.fill_rect c (0, 0) (vc.get_value(), 19));
      handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
                begin match Gctx.event_type e with
                | Gctx.MouseDown -> vc.change_value (fst (Gctx.event_pos e g))
                | Gctx.MouseDrag -> vc.change_value (fst (Gctx.event_pos e g))
                | _ -> ()
                end);
      size = fun () -> (100, 20)
    } in
    (w, vc))
    else failwith "negative slider input"
